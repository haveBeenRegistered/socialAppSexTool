...
def fetch_with_playwright(url, headers=None):
    ...
    with sync_playwright() as p:
        ...
        print("[fetch] page.goto completed")
...
def get_amazon_search_results():
    ...
    headers = {
        ...
        "Accept-Language": "ja-JP,zh;q=0.9,ja-JP;q=0.8,zh;q=0.7,zh-CN;q=0.6"
    }

    print("请求 URL →", url)
    # 仅使用无头浏览器发送请求
    print("尝试使用无头浏览器抓取页面…")
    page_html = fetch_with_playwright(url, headers)
    ...
    with open("/app/python/pagination.txt", "r+", encoding="utf-8") as pf:
        lines = pf.readlines()
        if lines:
            lines[0] = new_value
        else:
    ...
    if len(a_list_items) > 2:
        ...
        with open("/app/python/pagination.txt", "w", encoding="utf-8") as pf:
            pf.write(new_value)
            for url in urls:
                pf.write(url + "\n")
